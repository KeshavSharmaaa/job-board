  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F4F7FE] font-sans flex flex-col pt-[73px]">
        {/* Main Container */}
        <div className={`flex-1 max-w-[95rem] mx-auto w-full flex flex-col md:flex-row h-[calc(100vh-73px)] overflow-hidden ${user.role === 'candidate' ? 'gap-6 p-6' : ''}`}>

          {user.role === 'candidate' && <CandidateSidebar activeTab="browse" />}

          {/* Dual-Pane Inner Container */}
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-white rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  jobs.map((job) => {
                    ...
                    return (
                      <div
                        ...
                        className={`cursor-pointer rounded-2xl p-5 border transition-all duration-300 relative overflow-hidden group ${isSelected
